...
static void render_callback(Canvas* const canvas, void* ctx) {
    ...
    canvas_draw_str_aligned(canvas, plugin_state->x, plugin_state->y, AlignRight, AlignBottom, "Hack The Planet");
...
int32_t test_app(void* p) {
    ...
    if (!init_mutex(&state_mutex, plugin_state, sizeof(PluginState))) {
        FURI_LOG_E("TestApp", "cannot create mutex\r\n");
        free(plugin_state);
        return 255;
    }
    ...
    for(bool processing = true; processing;) {
        ...
        if(event_status == osOK) {
            ...
            // event timeout
            FURI_LOG_D("TestApp", "osMessageQueue: event timeout");
        }
